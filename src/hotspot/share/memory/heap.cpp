...
void CodeHeap::mark_segmap_as_free(size_t beg, size_t end) {
  ...
  assert(beg < end, "sanity check");
  assert(end <= _number_of_committed_segments, "sanity check");
...
HeapBlock* CodeHeap::create_new_heapblock(size_t begin, size_t end) {
  assert(end <= _next_segment, "sanity check");
  assert(end > begin,"sanity check");
...
HeapBlock* CodeHeap::create_new_freeblock(size_t begin, size_t end) {
  assert(end > begin,"sanity check");
  // log_debug(topn)("create_new_freeblock: [%d, %d), freelist_length: %d", (int)begin, (int)end, _freelist_length);
  // tty->print_cr("create_new_freeblock: [%d, %d), freelist_length: %d", (int)begin, (int)end, _freelist_length);
  HeapBlock* block = block_at(begin);
  block->set_free();
  block->set_length(end-begin);
  mark_segmap_as_used(begin, end, false);
  // TODO: add some logic to maintain, read deallocate && deallocate_tail
  // add_to_freelist decrease _blob_count
  // but we do not delete any blob, we just add a freeblock, 
  // so we need increase blob_count to counteract the decreasing _blob_count done by add_to_freelist
  _blob_count++;
...
HeapBlock* CodeHeap::split_block_with_range(FreeBlock *b, size_t begin, size_t end, FreeBlock* prev) {
  // tty->print_cr("split_block_with_range");
  if (b == NULL) return NULL;
 ...
 * @return NULL, if no one was found
 */
HeapBlock* CodeHeap::search_block_in_freelist(size_t begin, size_t end) {
...
void* CodeHeap::allocate_at_offset (size_t size, size_t offset){
  ...
  if (end < _number_of_committed_segments) {
    ...
    // mark 这里可能需要改成，begin = next segment，分配新的空间。
    if (begin < _next_segment) {
      ...
      end = number_of_segments + _next_segment;
      begin = _next_segment;
      if (end > _number_of_committed_segments) {
        return NULL;
      }
    }
    else if (begin > _next_segment) {
      // add a free block from _next_segment to begin
      HeapBlock* newb = create_new_freeblock(_next_segment, begin);
      // create_new_freeblock 里面已经有_freelist_segments这个的增加操作
